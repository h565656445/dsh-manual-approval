...
将 `.dsh` 目录复制到 `$DSH_HOME/.agent-presets/hermes-manual-approval/`：

```powershell
# Windows PowerShell 7
$dst = Join-Path $env:DSH_HOME ".agent-presets\hermes-manual-approval"
Copy-Item -Recurse -Force ".\dsh" $dst
```

重启 DeepSeek Harness 后，在会话预设列表选择 `Hermes 人工审批`。

### 方式二：仅安装技能（skill）

将 SKILL.md 复制到 `$DSH_HOME/skills/dsh-manual-approval/`：

```powershell
$dst = Join-Path $env:DSH_HOME "skills\dsh-manual-approval"
New-Item -ItemType Directory -Force -Path $dst | Out-Null
Copy-Item ".\skills\dsh-manual-approval\SKILL.md" $dst -Force
```
